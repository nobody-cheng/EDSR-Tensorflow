...
def get_test_set(original_size, shrunk_size):
    ...
    x = [scipy.misc.imresize(get_image(q, original_size), (shrunk_size, shrunk_size)) for q in
         imgs]
    y = [get_image(q, original_size) for q in
         imgs]
    return x, y
...
batch_size: size of the batch  批次的大小
original_size: size for target images  目标图片的大小
shrunk_size: size for shrunk images   缩略图的大小

returns x,y where:
-x is the input set of shape [-1,shrunk_size,shrunk_size,channels]   输入的形状
-y is the target set of shape [-1,original_size,original_size,channels] 输出的形状
"""
...
def get_batch(batch_size, original_size, shrunk_size):
    ...
    for i in range(len(img_indices)):
        ...
        if img.shape:
            ...
            y.append(img)
    """
    max_counter = int(len(train_set) / batch_size)
    print('{}/{}'.format(len(train_set), batch_size))
