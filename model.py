...
class EDSR(object):
    ...
    def __init__(self, img_size=32, num_layers=32, feature_size=256, scale=2, output_channels=3):
        ...
        # One convolution before res blocks and to convert to required feature depth 在res块之前进行一次卷积并转换为所需的特征深度
        x = slim.conv2d(image_input, feature_size, [3, 3])

        # Store the output of the first convolution to add later 存储第一个卷积的输出以后添加
        conv_1 = x
    ...
    def save(self, savedir='saved_models'):
        ...
        """
        self.saver.save(self.sess, savedir + "/model.ckpt")
        print("Saved!")
    ...
    def resume(self, savedir=''):
        """测试阶段使用saver.restore() 恢复变量"""
        print("Restoring...")
        # self.saver = tf.train.import_meta_graph('./saved_models/model.ckpt.meta')
